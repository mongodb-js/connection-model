...
Connection = AmpersandModel.extend({
  ...
  validateKerberos(attrs) {
    if (attrs.authentication !== 'KERBEROS') {
      ...
      throw new TypeError(format(
        'The kerberosPrincipal field is required when '
  ...
  validateX509(attrs) {
    if (attrs.authentication === 'X509') {
      if (!attrs.x509Username) {
        throw new TypeError(format(
          'The x509Username field is required when '
...
Connection.from = (url, callback) => {
  ...
  parseConnectionString(unescapedUrl, (error, parsed) => {
    ...
    if (attrs.options && attrs.options.replicaSet) {
      attrs.replicaSet = attrs.options.replicaSet;
    }

    if (attrs.options && attrs.options.readPreference) {
      attrs.readPreference = attrs.options.readPreference;
    }

    // The `authentication` value for humans
    let authentication = null;

    if (attrs.options && attrs.options.authMechanism) {
      authentication = attrs.options.authMechanism;
      ...
      if (attrs.authentication === 'LDAP') {
        ...
        if (attrs.options) {
          attrs.mongodbDatabaseName = decodeURIComponent(
            attrs.options.authSource || Connection.MONGODB_DATABASE_NAME_DEFAULT
          );
        }
