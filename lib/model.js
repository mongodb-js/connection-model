...
_.assign(derived, {
  ...
  driver_url: {
    ...
    fn: function() {
      ...
      let result = toURL(req);
      if (this.ssh_tunnel !== 'NONE') {
        // TODO: This incorrectly updates the Connect dialog form values
        // req.hostname = this.ssh_tunnel_options.localAddr;
        // req.port = this.ssh_tunnel_options.localPort;
        // TODO: Horribly fragile but works :(
        result = result
            .replace(req.hostname, this.ssh_tunnel_options.localAddr)
            .replace(req.port, this.ssh_tunnel_options.localPort);
      }
      return result;
    }
  },
  ...
  ssh_tunnel_options: {
    ...
    fn: function() {
      ...
      // Update connection to mongo to use local SSH Tunnel
      // debugger;
      // this.hostname = opts.localAddr;
