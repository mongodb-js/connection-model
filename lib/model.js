...
Connection = AmpersandModel.extend({
  ...
  initialize(attrs) {
     ...
     *
     * @param {Object} cAttrs - The connection attributes from url.
     */
    const parseAttrs = (cAttrs) => {
      if (cAttrs.sslCA && !Array.isArray(cAttrs.sslCA)) {
        this.sslCA = cAttrs.sslCA = [cAttrs.sslCA];
      }

      if (cAttrs.sshTunnel && cAttrs.sshTunnel !== 'NONE') {
        const port = localPortGenerator();

        cAttrs.sshTunnelBindToLocalPort = port;
