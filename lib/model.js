...
/**
 * Assigning observable properties of a state class
 */
assign(props, {
  /**
...
assign(derived, {
  ...
  authentication: {
    ...
    fn() {
      ...
      if (this.options && this.options.authMechanism) {
        authentication = this.options.authMechanism;
      } else if (this.auth && this.auth.username && this.auth.password) {
        authentication = 'DEFAULT';
      } else if (this.auth && this.auth.username) {
 ...
 */
assign(props, {
  ssl: { type: 'string', values: SSL_VALUES, default: SSL_DEFAULT },
  /**
  ...
  driverUrl: {
    ...
    fn() {
      ...
      if (this.options.replicaSet) {
        req.query.replicaSet = this.options.replicaSet;
      }

      req.query.readPreference = this.readPreference;

      if (this.appname) {
        req.query.appname = this.appname;
      }
...
Connection = AmpersandModel.extend({
  ...
  parse(attrs) {
    ...
    if (attrs.authentication === 'KERBEROS') {
      ...
      this.kerberosServiceName = attrs.kerberos_service_name;
    }
  ...
  validateSsl(attrs) {
    ...
    if (attrs.ssl === 'SERVER' && !attrs.sslCA) {
...
Connection.from = (url, callback) => {
  ...
  parseConnectionString(unescapedUrl, (error, parsed) => {
    ...
    const attrs = Object.assign({}, parsed, {
    ...
    if (parsed.auth) {
      ...
      const password = parsed.auth.password;
      ...
      if (attrs.authentication === 'LDAP') {
        ...
        Object.assign(attrs, Connection._improveAtlasDefaults(url, parsed.auth.password, attrs.ns));
      }
    }
 ...
 * @param {String} mongodbPassword - The mongodb_password
