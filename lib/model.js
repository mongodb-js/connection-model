...
Connection = AmpersandModel.extend({
  ...
  initialize(attrs) {
     ...
     * with the proper sslCA and sshTunnelBindToLocalPort values.
     *
     * @param {Object} parsedAttrs - The parsed connection attributes.
     */
    const parseAttrs = (parsedAttrs) => {
      if (parsedAttrs.sslCA && !Array.isArray(parsedAttrs.sslCA)) {
        this.sslCA = parsedAttrs.sslCA = [parsedAttrs.sslCA];
      }

      if (parsedAttrs.sshTunnel && parsedAttrs.sshTunnel !== 'NONE') {
        const port = localPortGenerator();

        parsedAttrs.sshTunnelBindToLocalPort = port;
        this.sshTunnelBindToLocalPort = port;
      }

      this.parse(parsedAttrs);
    };

    if (attrs) {
      if (typeof attrs === 'string') {
        attrs = Connection.from(attrs, (error, c) => {
          if (error) {
