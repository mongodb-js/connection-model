...
function getTasks(model) {
  ...
  _.assign(tasks, {
    'Connect to MongoDB': function(cb) {
      ...
      MongoClient.connect(model.driver_url, driverConnectOptions, function(err, _db) {
        ctx(err);
        if (err) {
          if (tunnel) {
