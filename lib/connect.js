    ...
    if (Array.isArray(opts[key])) {
      opts[key].forEach(value => {
        if (typeof value === 'string') {
          tasks[key] = cb =>
            async.parallel(opts[key].map(k => fs.readFile.bind(null, k)), cb);
        }
      });
    }
...
const getTasks = (model, setupListeners) => {
  ...
  assign(tasks, {
    'Check reachable': cb => {
      const ctx = status('Check reachable', cb);
      const hostsReachable = {};

      Promise.all(
        model.hosts.map(hostport => {
          return isPortReachable(hostport.port, { host: hostport.host });
        })
      )
        .then(successes => {
          var assert = require('assert');
          assert.equal(successes.length, model.hosts.length);

          var failed = successes.filter((s, i) => {
            console.log('failed?', {
              reachable: s,
              index: i,
              hosts: model.hosts[i]
          ...
          if (failed.length === model.hosts.length) {
            throw new Error('Is mongodb actually running? Are you online?');
          }

          successes.filter((s, i) => {
            if (!s) {
              throw new Error(
                'Failed to reach hostport. Is your spelling correct?',
                model.hosts[i]
              );
            }
          });
  ...
  //   > Connection.from(url, (err, conn) => console.log(conn.hosts))
  // undefined
  // > [ { host: 'compass-data-sets-shard-00-00-e06dc.mongodb.net',
  //     port: 27017 },
