    ...
    [Tasks.ConnectToMongoDB]: (cb) => {
      ...
      if (tunnel) {
        tunnel.on('error', (tunnelError) => {
          if (tunnelError.origin === 'ssh-client') {
      ...
      mongoClient.connect((err, _client) => {
        ...
        if (err) {
          if (tunnel) {
            ...
            tunnel.close().then(
              ...
              (tunnelError) => {
                debug('ssh tunnel stopped with error: %s', tunnelError.message);
                cb(err);
              }
            );
          }
        ...
        if (tunnel) {
          ...
          client.close = function close(force, callback) {
            ...
            const promise = new Promise((res, rej) => {
              function callbackOrPromise(error = null) {
                if (callback) {
              ...
              origClose.call(this, force, (clientError) => {
                ...
                tunnel.close().then(
                  ...
                  (tunnelError) => {
                    debug(
                      'ssh tunnel stopped with error: %s',
                      tunnelError.message
                    );
                    callbackOrPromise(clientError || tunnelError);
                  }
                );
              });
            });
