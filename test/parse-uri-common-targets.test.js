...
describe('connection model should parse URI strings for common connection targets such as', () => {
  context('ATLAS - mongodb.net when a database is provided', () => {
    ...
    const okAtlasPasswordConnection = atlasConnection.replace('<PASSWORD>', okAtlasPassword);
    ...
    it('works with a non-default secure password', (done) => {
      ...
      const modifiedAtlasConnection = atlasConnection.replace('<PASSWORD>', userPass);
    ...
    it('is case insensitive, see RFC4343', (done) => {
      const modifiedAtlasConnection = atlasConnection.replace(/mongodb.net/g, 'mOnGOdB.NeT');
  ...
  context('ATLAS - mongodb.net when a database is not provided', () => {
    ...
    it('sets hostname, port, ns, authSource', (done) => {
      Connection.from(atlasConnection, (error, result) => {
        expect(error).to.not.exist;
        expect(result.hostname).to.be.equal('a-compass-atlas-test-shard-00-00-vll9l.mongodb.net');
